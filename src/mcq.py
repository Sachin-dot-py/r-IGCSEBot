...
async def build_question(interaction: discord.Interaction, question_pages, question_number: int) -> Question | None:
    if question_number >= len(question_pages):
        await interaction.send(content="You have already reached the last question!", ephemeral=True)
        return
    ...
    try:
        ...
        if view.is_valid:
            ...
            await interaction.send(content=f"Question {question_number + 1} contains data that is too complex to display!", ephemeral=True)
...
class SkipButon(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.author.id != interaction.user.id:
            await interaction.send(content="This is not for you!", ephemeral=True)
            return
        question = await build_question(interaction, self.question_pages, self.question_number + 1)
        if isinstance(question, Question):
            await interaction.edit(embed=question.embed, view=question.view)

class Choice(discord.ui.View):
    def __init__(self, interaction: discord.Interaction, options, question_pages, question_number: int):
        super().__init__()
        self.correct_choice = order.index(question_pages[question_number][0]["correctChoice"])
