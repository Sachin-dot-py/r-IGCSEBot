...
async def Instantlockcommand(
    ...
    if action_type == "Forum Lock":
        if threadinput == None:
            await interaction.send(
                f"Please mention the forum post in the `thread_name` field.",
                ephemeral=True,
            )
        else:
    ...
    if action_type == "Channel Lock":
        if channelinput == None:
            await interaction.send(
                f"Please mention the channel in the `channel_name` field.",
                ephemeral=True,
            )
        else:
            ...
            if (
                ...
                and overwrite.send_messages_in_threads == True
            ):
