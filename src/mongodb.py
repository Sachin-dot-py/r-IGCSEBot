...
class GuildPreferencesDB:
    ...
    def set_pref(self, pref: str, pref_value, guild_id: int):
        if self.pref.find_one({"guild_id": guild_id}):
            result = self.pref.update_one(
                {"guild_id": guild_id}, {"$set": {pref: pref_value}}
            )
        else:
    ...
    def add_rep(self, user_id: int, guild_id: int):
        ...
        if rep is None:
            ...
            self.reputation.insert_one(
                {"user_id": user_id, "guild_id": guild_id, "rep": rep}
            )
        else:
            rep += 1
